...
[Mocha](https://mochajs.org/) is used to test Sibmei's output from a set of test files. Tests  use XPath to query the test files.  
After exporting the test file set with sibmei (Testsibmei will automatically do that), run `npm test` from the root directory of this git repository.
